...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'Redirect', fields ['uri', 'record']
        db.delete_unique('web_redirect', ['uri', 'record_id'])

        # Removing unique constraint on 'Redirect', fields ['name', 'record']
        db.delete_unique('web_redirect', ['name', 'record_id'])

        # Deleting model 'Redirect'
        db.delete_table('web_redirect')

        # Adding model 'Hosting'
        db.create_table('web_hosting', (
        ...
        db.send_create_signal('web', ['Hosting'])

        # Adding unique constraint on 'Hosting', fields ['name', 'record']
        db.create_unique('web_hosting', ['name', 'record_id'])

        # Adding unique constraint on 'Hosting', fields ['uri', 'record']
        db.create_unique('web_hosting', ['uri', 'record_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Hosting', fields ['uri', 'record']
        db.delete_unique('web_hosting', ['uri', 'record_id'])

        # Removing unique constraint on 'Hosting', fields ['name', 'record']
        db.delete_unique('web_hosting', ['name', 'record_id'])

        # Adding model 'Redirect'
        db.create_table('web_redirect', (
            ('name', self.gf('django.db.models.fields.TextField')(max_length=60, unique=True)),
            ('url', self.gf('django.db.models.fields.TextField')()),
            ('uri', self.gf('django.db.models.fields.TextField')()),
            ('record', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['dns.Records'])),
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
        ))
        db.send_create_signal('web', ['Redirect'])

        # Adding unique constraint on 'Redirect', fields ['name', 'record']
        db.create_unique('web_redirect', ['name', 'record_id'])

        # Adding unique constraint on 'Redirect', fields ['uri', 'record']
        db.create_unique('web_redirect', ['uri', 'record_id'])

        # Deleting model 'Hosting'
        db.delete_table('web_hosting')
    ...
    models = {
        ...
        'web.redirectdynhost': {
            'Meta': {'unique_together': "(('name', 'dynhost'), ('uri', 'dynhost'))", 'object_name': 'RedirectDynHost'},
